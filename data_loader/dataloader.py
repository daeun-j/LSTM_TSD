...
class LSTMTSD_Dataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        x = torch.from_numpy(train_data).type(torch.float)
        y = torch.from_numpy(target_data).type(torch.float)
        # TODO : meta = stat, fft, arima, between
        return x, y
...
if __name__ == '__main__':
    df = pd.read_csv('../dataset/Telegram_1hour_7.csv', header=0)
    df = df[["Time", "Length"]].to_numpy().round()
    df_set = LSTMTSD_Dataset(df, window_size=10, horizon=3, normalize_method="z_score")
    train_loader = DataLoader(df_set, batch_size=2, drop_last=False, shuffle=True, num_workers=0)
