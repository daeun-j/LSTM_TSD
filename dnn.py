...
name = "jrnl_DNN_eps{}_merge{}_w{}".format(args.num_epochs, args.MERGE, args.window_size)
name_merge = "merge{}".format(args.MERGE)
hyper_params = {"fft": args.fft, "stat" : args.stat, "MERGE" : args.MERGE,"num_epochs": args.num_epochs,
                "window_size": args.window_size,"lr" : args.lr, "batch_size" : args.batch_size
...
for epoch in range(args.num_epochs):
    ...
    for tr_i, (inputs, labels) in enumerate(train_loader):
        ...
        if tr_i % 300 == 0:
            print(f'Train Epoch {tr_i + 0:04}: | Train Loss: {train_loss:.5f} | Train Acc: {train_acc:.3f}')
    ...
    print(str(epoch)+"_valid_time: ", valid_time)
